...
#| fig-cap: "Ensemble of 100 simulations for each scenario showing catch variability under constant F and the FSPR HCR. Thin lines are individual simulations and thick solid lines are medians."
#| fig-height: 5
#| fig-width: 10
...
To stress-test harvest rates, we run each scenario at constant fishing mortality values spanning 0.5-1.5 x $F_{MSY}$. @tbl-f-sensitivity lists the constant-F results alongside the FSPR HCR outcomes (with and without the 1.45 Mt cap), and @fig-f-sensitivity visualizes the tradeoffs.

```{r f-sensitivity-table}
#| label: tbl-f-sensitivity
#| tbl-cap: "Performance across constant F multipliers and FSPR HCR policies (100 simulations per level)."
...
Illustrative triggers that could be implemented in this simulation framework include:
...
- **Trend trigger**: if SSB declines for 4 of 5 consecutive years, switch to the capped HCR until SSB recovers above the threshold.
- **Catch variability trigger**: if catch CV exceeds a policy limit, activate the catch cap or a lower $F$ target.
- **Recruitment anomaly trigger**: if recruitment falls below a historical percentile for multiple years, tighten the ramp parameters.
